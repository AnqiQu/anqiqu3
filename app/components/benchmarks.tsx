    ...
    <div className="benchmark-chart-card">
      ...
      <ul className="benchmark-legend">
        {models.map((model) => (
          <li
            key={model.name}
            className={model.tone === "anqi" ? "benchmark-legend-anqi" : undefined}
          >
            <span
            ...
            <span className="benchmark-legend-score">avg {average(model.scores)}</span>
          </li>
        ))}
      </ul>
    </div>
